...
QMYSQLResult::QMYSQLResult(const QMYSQLDriverEmbedded* db)
: QSqlResult(db)
{
...
# if MYSQL_VERSION_ID >= 40000
    ...
    {
        setenv( "MYSQL_HOME", mysqlHome.toLocal8Bit().constData(), 1 );
        setenv( "DEFAULT_HOME_ENV", mysqlHome.toLocal8Bit().constData(), 1 );
    }
  
    char **server_options =new char* [mysqlServerOptions.size()+1 ];
        
    for(int i=0;i<mysqlServerOptions.size();i++)
    {
        const QString &s=mysqlServerOptions.at(i);
        if(s.isNull())
        {
             server_options[i]=0;
        }
        ...
        {    
            server_options[i]=new char[strlen(s.toStdString().c_str())+1];
            strcpy(server_options[i],s.toStdString().c_str());        
        }
    }

# if (MYSQL_VERSION_ID >= 40110 && MYSQL_VERSION_ID < 50000) || MYSQL_VERSION_ID >= 50003
    if (mysql_library_init(mysqlServerOptions.size()-1, server_options, 0)) {
# else
    if (mysql_server_init(0, 0, 0)) {
# endif
        qWarning("QMYSQLDriverEmbedded::qServerInit: unable to start server.");
    }
...
QMYSQLDriverEmbedded::QMYSQLDriverEmbedded(QObject * parent)
    : QSqlDriver(parent)
{
    init();
    qLibraryInit();
...
QMYSQLDriverEmbedded::QMYSQLDriverEmbedded(MYSQL * con, QObject * parent)
    : QSqlDriver(parent)
{
    init();
    if (con) {
...
void QMYSQLDriverEmbedded::init()
{
...
QMYSQLDriverEmbedded::~QMYSQLDriverEmbedded()
{
    qMySqlConnectionCount--;
...
bool QMYSQLDriverEmbedded::hasFeature(DriverFeature f) const
{
    switch (f) {
    case Transactions:
...
static void setOptionFlag(uint &optionFlags, const QString &opt)
{
    if (opt == QLatin1String("CLIENT_COMPRESS"))
        ...
        qWarning("QMYSQLDriverEmbedded::open: Unknown connect option '%s'", opt.toLocal8Bit().constData());
}

bool QMYSQLDriverEmbedded::open(const QString& db,
                         const QString& user,
    ...
    */
    ...
    for (int i = 0; i < opts.count(); ++i) {
        ...
        if ((idx = tmp.indexOf(QLatin1Char('='))) != -1) {
            ...
            if (opt == QLatin1String("UNIX_SOCKET"))
                ...
                qWarning("QMYSQLDriverEmbedded::open: Illegal connect option value '%s'",
    ...
    if ((d->mysql = mysql_init((MYSQL*) 0)) &&
            /*mysql_real_connect(d->mysql,
                               ...
                               optionFlags))*/
            mysql_real_connect( d->mysql,NULL, NULL, NULL, 
                                     db.isNull() ? static_cast<const char *>(0)
                                                 : db.toLocal8Bit().constData(), 0, NULL, 0) )
    {
...
void QMYSQLDriverEmbedded::close()
{
    if (isOpen()) {
         mysql_close(d->mysql);
#ifndef QT_NO_THREAD
...
QSqlResult *QMYSQLDriverEmbedded::createResult() const
{
    return new QMYSQLResult(this);
}

QStringList QMYSQLDriverEmbedded::tables(QSql::TableType type) const
{
    QStringList tl;
#if MYSQL_VERSION_ID >= 40100
...
QSqlIndex QMYSQLDriverEmbedded::primaryIndex(const QString& tablename) const
{
    QSqlIndex idx;
    if (!isOpen())
        return idx;
...
QSqlRecord QMYSQLDriverEmbedded::record(const QString& tablename) const
{
...
QVariant QMYSQLDriverEmbedded::handle() const
{
    return QVariant::fromValue(d->mysql);
}

bool QMYSQLDriverEmbedded::beginTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        qWarning("QMYSQLDriverEmbedded::beginTransaction: Database not open");
        return false;
    }
...
bool QMYSQLDriverEmbedded::commitTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        qWarning("QMYSQLDriverEmbedded::commitTransaction: Database not open");
        return false;
    }
...
bool QMYSQLDriverEmbedded::rollbackTransaction()
{
#ifndef CLIENT_TRANSACTIONS
    return false;
#endif
    if (!isOpen()) {
        qWarning("QMYSQLDriverEmbedded::rollbackTransaction: Database not open");
        return false;
    }
...
QString QMYSQLDriverEmbedded::formatValue(const QSqlField &field, bool trimStrings) const
{
    QString r;
    if (field.isNull()) {
        ...
        case QVariant::ByteArray:
            if (isOpen()) {
                ...
                qWarning("QMYSQLDriverEmbedded::formatValue: Database not open");
            }
            // fall through
        default:
            r = QSqlDriver::formatValue(field, trimStrings);
...
QString QMYSQLDriverEmbedded::escapeIdentifier(const QString &identifier, IdentifierType) const
{
...
bool QMYSQLDriverEmbedded::isIdentifierEscapedImplementation(const QString &identifier, IdentifierType type) const
{
    Q_UNUSED(type);
...
QMYSQLResultPrivate::QMYSQLResultPrivate(const QMYSQLDriverEmbedded* dp, const QMYSQLResult* d) : driver(dp), result(0), q(d), 
...
QStringList mysqlServerOptions;
QString mysqlHome;