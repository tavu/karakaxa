...
database::abstractQuery* smplaylistItem::group(QDomNode nod) const
{
    database::matchQuery *q;
    ...
    if (s==QString("all") )
    {
        q=new database::matchQuery(database::AND);
    }
    else if (s==QString("any") )
    {
        q=new database::matchQuery(database::OR);
    }
    else
    {
        return 0;
...
//      q+=group(n);
        ...
        else if (name==QString("field") )
        {
            ...
            q->append(new database::tagQuery(t,comp,s,invert) );
...
database::abstractQuery* smplaylistItem::query() const
{
